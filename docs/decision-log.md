...
The Docker deployment uses `PALWORLD_LIVE_SAVE_LINUX_DIR` for both app configuration and host-side Compose interpolation. Compose mounts that Linux save path read-only at the same path inside the dashboard container, while direct Windows development uses `PALWORLD_LIVE_SAVE_WINDOWS_DIR`. Stable container paths remain `/data/pals` for persistent application data and `/work/decode-work` for the parser sidecar's view of the shared workspace. The dashboard image includes parser metadata and image assets from the upstream v0.12.2 commit `3b0e1e96a7500846e3a6fbac66f1c248b4c286e7`. The parser image is pinned to its multi-platform digest.
